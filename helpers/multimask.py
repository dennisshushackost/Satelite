...
class ProcessMask:
    ...
    def __init__(self, data_path, parcel_index, all_class_ids, upscaled=False):
    ...
    def create_multiclass_mask(self):
        """
        Creates a multi-class mask with different class IDs.
        """
        try:
            with rasterio.open(self.satellite_path) as src:
                meta = src.meta.copy()
            ...
            for class_id in self.all_class_ids:
                if class_id in self.parcel['class_id'].unique():
                    class_mask = rasterize(
                        shapes=self.parcel[self.parcel['class_id'] == class_id].geometry,
                        out_shape=(meta['height'], meta['width']),
                        fill=0,
                        transform=meta['transform'],
                        all_touched=True,
                    ...
                    # Create an empty mask for the class if it doesn't exist in the parcel
                    class_mask = np.zeros((meta['height'], meta['width']), dtype=rasterio.uint8)
                masks.append(class_mask)

            # Write the multi-band mask to a file
            with rasterio.open(self.mask_path / self.mask_name, 'w', **meta) as out:
                for i, mask in enumerate(masks, start=1):
                    out.write(mask, i)
        except Exception as e:
            print(f'Error creating multi-class mask: {e}')
