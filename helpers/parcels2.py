...
class ProcessParcels:
    ...
    """
    def __init__(self, data_path, trimmed=False, combine_adjacent=False):
    ...
    def process_parcels(self):
        ...
        for image_file in tqdm(satellite_images, desc='Processing parcels'):
            ...
            if self.combine_adjacent:
                # Combine adjacent parcels with the same 'nutzung' type
                combined_parcels = self.combine_adjacent_parcels(trimmed_parcels)
                # Save combined parcels
                combined_output_path = os.path.join(self.parcel_data_path, new_file_name.split('.')[0] + "_combined.gpkg")
                combined_parcels.to_file(combined_output_path, driver="GPKG")
...
if __name__ == "__main__":
    ...
    grid = ProcessParcels(data_path, trimmed=False, combine_adjacent=True)
